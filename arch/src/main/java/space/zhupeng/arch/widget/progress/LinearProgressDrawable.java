...
public class LinearProgressDrawable extends Drawable implements Animatable {
    ...
    public void draw(Canvas canvas) {
        switch (mProgressMode) {
            case MaterialProgressView.MODE_DETERMINATE:
                drawDeterminate(canvas);
                break;
            case MaterialProgressView.MODE_INDETERMINATE:
                drawIndeterminate(canvas);
                break;
            case MaterialProgressView.MODE_BUFFER:
                drawBuffer(canvas);
                break;
            case MaterialProgressView.MODE_QUERY:
                drawQuery(canvas);
                break;
        }
    }
    ...
    private int getQueryStrokeColor() {
        int alpha = Math.round(Color.alpha(mStrokeColors[0]) * mAnimTime);
        return (mStrokeColors[0] & 0x00FFFFFF) | (alpha << 24);
    }
    ...
    private void resetAnimation() {
        ...
        if (mProgressMode == MaterialProgressView.MODE_INDETERMINATE) {
        ...
        } else if (mProgressMode == MaterialProgressView.MODE_BUFFER) {
            mStartLine = 0;
        } else if (mProgressMode == MaterialProgressView.MODE_QUERY) {
            mStartLine = !mReverse ? getBounds().width() : 0;
            mStrokeColorIndex = 0;
    ...
    private void start(boolean withAnimation) {
        ...
        scheduleSelf(mUpdater, SystemClock.uptimeMillis() + MaterialProgressView.FRAME_DURATION);
        invalidateSelf();
    }
    ...
    public static class Builder {
        ...
        private Interpolator mTransformInterpolator;
        private int mProgressMode = MaterialProgressView.MODE_INDETERMINATE;
